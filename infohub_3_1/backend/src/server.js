...
async function start() {
  ...
  if (conectado) {
    try {
      ...
      `);
      // Migracao v2: cria as tabelas/colunas dos requisitos complementares.
      const arquivoMigracao = path.join(__dirname, "..", "db", "002_v2.sql");
      if (fs.existsSync(arquivoMigracao)) {
        try {
          await sequelize.query(fs.readFileSync(arquivoMigracao, "utf8"));
          console.log("Migracao 002_v2.sql aplicada com sucesso.");
        } catch (migracaoErro) {
          console.error("Falha ao aplicar 002_v2.sql:", migracaoErro.message);
        }
      }
