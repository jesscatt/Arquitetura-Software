...
// Sessao atual (usada pelo front-end ao restaurar o token guardado).
router.get("/me",autenticar,async(req,res)=>{
  ...
  res.json({
    ...
    telefone:usuario.telefone||null,curso:usuario.curso||null,semestre:usuario.semestre||null,
  });
});

// RF-21 - preferencias de notificacao por e-mail do proprio usuario.
router.patch("/preferencias",autenticar,async(req,res,next)=>{
  try{
    const dados={};
    if(req.body.notificacoesEmail!==undefined) dados.notificacoesEmail=Boolean(req.body.notificacoesEmail);
    for(const campo of ["telefone","curso","semestre"]){
      if(req.body[campo]!==undefined) dados[campo]=String(req.body[campo]).trim().slice(0,120)||null;
    }
    if(!Object.keys(dados).length) return res.status(400).json({erro:"Nenhuma preferência informada."});
    await req.usuario.update(dados);
    res.json({ok:true,notificacoesEmail:req.usuario.notificacoesEmail!==false});
  }catch(e){next(e);}
});
