...
function showSection(section) {
  ...
  $("#pageTitle").textContent = section === "teams" ? "Equipes" : section === "tasks" ? "Tarefas" : section === "reports" ? "Relatórios" : section === "audit" ? "Logs de auditoria" : section === "users" ? "Usuários" : section === "profiles" ? "Perfis e permissões" : "Administração";
...
async function renderAuditPage() {
  elements.workspaceView.innerHTML = `<div class="workspace-heading"><div><span class="eyebrow">SEGURANÇA</span><h2>Logs de auditoria</h2><p>Histórico das ações realizadas pelos usuários no sistema.</p></div><button class="secondary-button" id="refreshAudit">↻ Atualizar</button></div><div class="data-table" id="auditRows"><div class="empty-column">Carregando registros…</div></div>`;
  try {
    const logs = await api("/auditoria?limit=200");
    const el = $("#auditRows");
    el.innerHTML = logs.length ? logs.map(log => `<div class="data-row"><div><strong>${escapeHtml(log.usuario?.nome || "Sistema")}</strong><small>${escapeHtml(log.usuario?.email || "Ação automática")}</small></div><span>${escapeHtml(log.acao)}</span><span>${escapeHtml(log.modulo)}</span><span>${escapeHtml(log.metodo)}</span><span>${log.criadoEm ? new Date(log.criadoEm).toLocaleString("pt-BR") : "—"}</span></div>`).join("") : '<div class="empty-column">Nenhum registro de auditoria encontrado.</div>';
    $("#refreshAudit")?.addEventListener("click", renderAuditPage);
  } catch (e) { $("#auditRows").innerHTML = `<div class="empty-column">Não foi possível carregar os logs: ${escapeHtml(e.message)}</div>`; }
}
