...
function applyNavigationPermissions(){ const map={teams:"equipes.visualizar",tasks:"tarefas.visualizar",reports:"relatorios.visualizar",audit:"auditoria.visualizar",settings:"configuracoes.visualizar",users:"usuarios.visualizar",profiles:"usuarios.visualizar"}; document.querySelectorAll(".nav-item").forEach(btn=>{const p=map[btn.dataset.section]; if(p) btn.classList.toggle("visible",can(p));}); }
...
async function api(path, options = {}) {
  ...
  if (!response.ok) throw new Error(data?.erro || data?.detalhe || `Erro ${response.status}`);
  return data;
}
...
function showSection(section) {
  ...
  $("#pageTitle").textContent = section === "teams" ? "Equipes" : section === "tasks" ? "Tarefas" : section === "reports" ? "Relatórios" : section === "audit" ? "Logs de auditoria" : section === "users" ? "Usuários" : section === "profiles" ? "Perfis e permissões" : section === "settings" ? "Configurações" : "Administração";
...
function renderWorkspace(section) {
  ...
  if (section === "settings") return renderConfigPage();
  if (section === "users") return renderSettingsPage("users");
  if (section === "profiles") return renderSettingsPage("profiles");
  if (section === "reports") return renderReportsPage();
  if (section === "audit") return renderAuditPage();
  renderReportsPage();
}
...
function renderConfigPage(){
  const savedTheme = localStorage.getItem("infohub-theme") || "dark";
  elements.workspaceView.innerHTML = `<div class="workspace-heading"><div><span class="eyebrow">ADMINISTRAÇÃO</span><h2>Configurações</h2><p>Preferências gerais do ambiente administrativo e parâmetros do ciclo atual.</p></div></div>
    <div class="config-grid">
      <section class="config-card"><div class="config-card-head"><div><span class="settings-nav-icon">◐</span><h3>Aparência</h3><p>Escolha como o InfoHub será exibido neste navegador.</p></div></div>
        <div class="config-option"><div><strong>Tema da interface</strong><small>A mesma preferência é usada no login e no painel.</small></div><div class="theme-choice"><button type="button" data-config-theme="light" class="${savedTheme==='light'?'active':''}">Claro</button><button type="button" data-config-theme="dark" class="${savedTheme==='dark'?'active':''}">Escuro</button></div></div>
      </section>
      <section class="config-card"><div class="config-card-head"><div><span class="settings-nav-icon">✉</span><h3>Notificações por e-mail</h3><p>Os lembretes são enviados por e-mail conforme as tarefas e prazos configurados.</p></div></div>
        <div class="config-list"><div><strong>Convites de acesso</strong><span class="config-status">Automático</span></div><div><strong>Novas tarefas e alterações</strong><span class="config-status">Automático</span></div><div><strong>Prazo próximo ou vencido</strong><span class="config-status">Configurável por tarefa</span></div></div>
      </section>
      <section class="config-card"><div class="config-card-head"><div><span class="settings-nav-icon">▦</span><h3>Ciclo do programa</h3><p>Referência visual para o acompanhamento das equipes.</p></div></div>
        <label class="config-field">Ciclo ativo<input value="2026/2" aria-label="Ciclo ativo" /></label>
        <div class="info-callout"><strong>Parâmetros avançados</strong><small>Áreas da ideia, modelos de tarefas e regras de lembrete devem ser configurados pelo módulo de administração quando a persistência desses parâmetros estiver disponível.</small></div>
      </section>
    </div>`;
  document.querySelectorAll('[data-config-theme]').forEach(btn=>btn.addEventListener('click',()=>{
    const theme=btn.dataset.configTheme; localStorage.setItem('infohub-theme',theme); applyTheme(theme); renderConfigPage(); showToast(`Tema ${theme==='light'?'claro':'escuro'} aplicado.`);
  }));
}

function renderAuditPage(){
  elements.workspaceView.innerHTML = `<div class="workspace-heading"><div><span class="eyebrow">RASTREABILIDADE</span><h2>Logs de auditoria</h2><p>Área reservada para acompanhar mudanças de etapa, aprovações, reprovações e envios de e-mail.</p></div><button class="secondary-button" id="auditRefresh">↻ Atualizar</button></div>
    <section class="audit-panel"><div class="audit-empty"><div class="settings-empty-icon">◷</div><h3>Registro de auditoria</h3><p>A estrutura da navegação está pronta. O backend atual ainda não possui uma tabela/rota de auditoria persistida, então nenhum evento é inventado nesta tela.</p><span>Próxima implementação: persistir eventos com usuário, data/hora, ação, recurso e resultado.</span></div></section>`;
  $("#auditRefresh")?.addEventListener("click",()=>showToast("Os logs serão carregados quando o módulo de auditoria persistente estiver habilitado."));
}
...
function renderReportsPage() {
  ...
  $("#exportReports")?.addEventListener("click",()=>{
    const rows=[["Equipe","Etapa","Status","Tarefas","Tarefas atrasadas"]];
    cache.equipes.map(normalizeTeam).forEach(team=>{const tasks=cache.tarefas.filter(t=>Number(t.equipeId)===Number(team.id));rows.push([team.name,team.currentStage.number,team.status,tasks.length,tasks.filter(t=>t.status==='atrasada').length]);});
    const csv=rows.map(r=>r.map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(';')).join('\n');
    const blob=new Blob(["\ufeff"+csv],{type:'text/csv;charset=utf-8;'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='infohub-relatorio-equipes.csv';a.click();URL.revokeObjectURL(url);showToast('Relatório CSV exportado.');
  });
}
...
async function enterApp(event) {
  ...
  try {
    ...
    $("#userName") && ($("#userName").textContent = currentUser.name); $("#userRole") && ($("#userRole").textContent = currentUser.role === "ADMIN" ? "Administrador" : currentUser.role);
    if ($("#userAvatar")) $("#userAvatar").innerHTML = avatarMarkup(currentUser.name, currentUser.id, "user-avatar").replace(/^<span class="user-avatar"/, '<span class="user-avatar"');
    const roleLabelTop = currentUser.role === "ADMIN" ? "Administrador" : currentUser.role;
    ["#topAvatar", "#topAvatarBig"].forEach(selector => {
      const el = $(selector);
      if (el) el.outerHTML = avatarMarkup(currentUser.name, currentUser.id, "mini-avatar");
    });
...
function doLogout() { currentUser = null; accessToken = null; elements.appShell.classList.add("is-hidden"); elements.loginScreen.classList.remove("is-hidden"); elements.loginForm.reset(); $("#email").value = "admin@infohub.com"; }
...
function setupUserMenu() {
  ...
  $("#menuProfileButton")?.addEventListener("click", () => { closeMenu(); showToast("Edição de perfil em breve."); });
...
$("#appNotificationButton")?.addEventListener("click", () => showToast("As notificações do InfoHub são enviadas por e-mail. Os próximos avisos aparecerão aqui quando o centro de notificações for ativado."));
...
document.querySelectorAll(".nav-item").forEach((button) => button.addEventListener("click", () => { const needed={teams:"equipes.visualizar",tasks:"tarefas.visualizar",reports:"relatorios.visualizar",audit:"auditoria.visualizar",settings:"configuracoes.visualizar",users:"usuarios.visualizar",profiles:"usuarios.visualizar"}[button.dataset.section]; if(needed&&!can(needed)){showToast("Seu perfil não possui acesso a esta área.");return;} showSection(button.dataset.section); }));
