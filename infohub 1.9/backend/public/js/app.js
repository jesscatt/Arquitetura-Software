...
function applyNavigationPermissions(){ const map={teams:"equipes.visualizar",tasks:"tarefas.visualizar",reports:"relatorios.visualizar",settings:"configuracoes.visualizar"}; document.querySelectorAll(".nav-item").forEach(btn=>{const p=map[btn.dataset.section]; if(p) btn.classList.toggle("visible",can(p));}); }
...
function teamCard(item) {
  ...
  return `<button class="team-card" type="button" data-team-id="${item.id}" aria-label="Abrir equipe ${escapeHtml(item.name)}">
    ...
    <div class="mentor-row"><span class="mini-avatar">${initials(item.mentor.name)}</span><span>${escapeHtml(item.mentor.name)}</span></div>
...
async function openTeam(id) {
  ...
  const setTab = (tab) => {
    ...
    body.querySelectorAll(".task-status-select").forEach(select => select.addEventListener("change", async () => {
      try { select.disabled = true; await updateTaskStatus(select.dataset.taskId, select.value); setTab(tab); } catch (error) { showToast(error.message); } finally { select.disabled = false; }
    }));
  ...
  elements.dialogContent.innerHTML = `<header class="dialog-header team-modal-header"><div><span class="eyebrow">DETALHE DA EQUIPE</span><h2>${escapeHtml(item.name)}</h2><p>${escapeHtml(item.project.name)} · ${escapeHtml(item.project.area.name)}</p></div><button class="close-button" id="closeDialog" type="button" aria-label="Fechar">×</button></header>
    ...
    <footer class="dialog-footer"><span>Equipe #${escapeHtml(item.id)}</span><button class="primary-button" id="advanceButton" type="button" ${item.currentStage.number >= 6 || !can("jornada.avancar") ? "disabled" : ""}>Avançar para próxima etapa →</button></footer>`;
...
function showSection(section) {
  ...
  $("#pageTitle").textContent = section === "teams" ? "Equipes" : section === "tasks" ? "Tarefas" : section === "reports" ? "Relatórios" : "Administração";
...
function renderWorkspace(section) {
  ...
  if (section === "settings") return renderSettingsPage();
  renderReportsPage();
}
...
function renderSettingsPage(){
  ...
  elements.workspaceView.innerHTML=`
    <div class="settings-hero">
      <div>
        <div class="settings-breadcrumb"><span>Administração</span><b>›</b><strong>Administração</strong></div>
        <div class="workspace-heading settings-heading"><div><span class="eyebrow">CENTRAL DE ADMINISTRAÇÃO</span><h2>Administração</h2><p>Gerencie usuários, acessos, perfis e permissões do InfoHub.</p></div><button class="primary-button" id="newUserButton">＋ Novo usuário</button></div>
      </div>
    </div>
...
function openNewUserDialog(user=null){
 ...
 loadProfiles().then(profiles=>{
  ...
  $("#userForm").addEventListener('submit',async e=>{
   ...
   data.ativo=form.ativo.checked; data.perfilId=Number(data.perfilId); if(!data.senha) delete data.senha;
   try{
    ...
    elements.dialog.close(); renderUsersPanel();
    showToast(result?.avisoEmail || (editing?'Usuário atualizado.':'Usuário criado e convite enviado.'));
   }catch(err){ showToast(err.message); }
   finally{ setLoading(button,false); }
  });
 }).catch(err=>showToast(err.message));
}
async function openEditUserDialog(id){const user=(cache.usuarios||[]).find(u=>Number(u.id)===Number(id));if(user)openNewUserDialog(user);}
async function toggleUser(id){const user=(cache.usuarios||[]).find(u=>Number(u.id)===Number(id));if(!user)return;try{if(user.ativo){await api(`/usuarios/${id}`,{method:'DELETE'});}else{await api(`/usuarios/${id}`,{method:'PATCH',body:JSON.stringify({ativo:true})});}await refreshData();renderUsersPanel();showToast(user.ativo?'Usuário desativado.':'Usuário ativado.');}catch(err){showToast(err.message);}}
...
async function enterApp(event) {
  ...
  try {
    ...
    $("#userName").textContent = currentUser.name; $("#userRole").textContent = currentUser.role === "ADMIN" ? "Administrador" : currentUser.role;
    $("#userAvatar").textContent = initials(currentUser.name);
    renderDashboard(); renderKanban(); renderDeadlines();
    document.querySelectorAll(".admin-only").forEach(el=>el.classList.toggle("visible", can("configuracoes.visualizar"))); applyNavigationPermissions();
...
$("#logoutButton").addEventListener("click", () => { currentUser = null; accessToken = null; elements.appShell.classList.add("is-hidden"); elements.loginScreen.classList.remove("is-hidden"); elements.loginForm.reset(); $("#email").value = "admin@infohub.com"; });
...
$("#notificationButton").addEventListener("click", () => { const overdue = cache.tarefas.filter((t) => t.status === "atrasada").length; showToast(overdue ? `Você tem ${overdue} tarefa(s) atrasada(s).` : "Nenhuma tarefa atrasada no momento."); });
document.querySelectorAll(".nav-item").forEach((button) => button.addEventListener("click", () => { const needed={teams:"equipes.visualizar",tasks:"tarefas.visualizar",reports:"relatorios.visualizar",settings:"configuracoes.visualizar"}[button.dataset.section]; if(needed&&!can(needed)){showToast("Seu perfil não possui acesso a esta área.");return;} showSection(button.dataset.section); }));
